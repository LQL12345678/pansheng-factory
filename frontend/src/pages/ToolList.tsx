...
import { Tool, ToolChangeRecord } from '../types';
...
export default function ToolList() {
  ...
  // 报废相关
  const [scrapTarget, setScrapTarget] = useState<Tool | null>(null);
  const [scrapForm, setScrapForm] = useState({ scrap_date: today(), scrap_reason: '', scrap_person: '' });
  ...
  // 计算分页数据
  const paginateData = useCallback((data: Tool[], page: number) => {
    const start = (page - 1) * PAGE_SIZE;
    const end = start + PAGE_SIZE;
    setTools(data.slice(start, end));
  ...
  const load = useCallback((keepPage?: number) => {
    toolApi.list().then(r => {
      const list = r.data;
      const filtered = search ? list.filter((t: Tool) =>
        (t.line || '').includes(search) ||
        (t.name || '').includes(search) ||
        (t.model || '').includes(search) ||
        (t.device_no || '').includes(search) ||
        (t.station || '').includes(search) ||
        String(t.quantity || '').includes(search)
      ) : list;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editTool?.id) {
        await toolApi.update(String(editTool.id), formData);
      } else {
        await toolApi.create(formData);
      }
      setShowForm(false); setEditTool(null); setFormData({}); load();
      alert(editTool ? '更新成功！' : '创建成功！');
    } catch (err: any) {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await toolApi.importExcel(file);
    alert('导入完成！'); load(); if (fileRef.current) fileRef.current.value = '';
  };

  const handleExport = () => {
    const exportData = tools.map((item: Tool) => ({
      '线体': item.line,
      '工具名称': item.name,
      '型号': item.model,
      '设备编码': item.device_no,
    ...
    XLSX.utils.book_append_sheet(wb, ws, '工具台账');
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([wbout], { type: 'application/octet-stream' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = '工具台账导出.xlsx'; a.click();
    URL.revokeObjectURL(url);
  };

  // ===== 报废 =====
  const openScrap = (d: Tool) => {
    setScrapTarget(d);
    setScrapForm({ scrap_date: today(), scrap_reason: '', scrap_person: '' });
  };
  ...
  const submitScrap = async (e: React.FormEvent) => {
    ...
    setScrapping(true);
    try {
      await toolApi.scrap(String(scrapTarget.id), scrapForm);
      setScrapTarget(null);
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      const res = await toolApi.changeRecords(params);
      setRecords(res.data);
    } catch {
      setRecords([]);
  ...
  return (
    <div>
      ...
      <div className="toolbar">
        {isAdmin && (
          <>
            <button className="primary" onClick={() => { setEditTool(null); setFormData({}); setShowForm(true); }}>＋ 新增工具</button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
          <input
            ...
            placeholder="搜索线体/名称/型号/设备编码/工位"
            value={search}
      ...
      <div className="card">
        <table>
          <thead><tr>
            <th>线体</th><th>工具名称</th><th>型号</th><th>设备编码</th><th>使用工位</th><th>数量</th><th>操作</th>
          </tr></thead>
          <tbody>
            {tools.map(d => {
              ...
              return (
                <tr key={d.id} style={isScrapped ? { opacity: 0.5 } : undefined}>
                  <td data-label="线体">{d.line || '-'}</td>
                  <td data-label="工具名称">{d.name}</td>
                  <td data-label="型号">{d.model || '-'}</td>
                  <td data-label="设备编码" className="monospace">{d.device_no || '-'}</td>
                  <td data-label="使用工位">{d.station || '-'}</td>
                  <td data-label="数量">{d.quantity ?? 1}</td>
                  <td>
                    {isScrapped ? (
                      <span style={{ color: '#999', fontSize: 12 }}>已报废</span>
                    ) : isAdmin ? (
                      <>
                        <button className="btn-sm" onClick={() => { setEditTool(d); setShowForm(true); }}>编辑</button>
      ...
      <ModalPortal visible={showForm}>
        <div className="modal-overlay">
          <div className="modal">
            <h3>{editTool ? '编辑工具' : '新增工具'}</h3>
            <form onSubmit={handleSubmit}>
              <div className="form-grid">
                <div><label>线体</label><input name="line" value={formData.line || ''} onChange={handleInputChange} placeholder="如：A线" /></div>
                <div><label>工具名称 *</label><input name="name" value={formData.name || ''} onChange={handleInputChange} required /></div>
                <div><label>型号</label><input name="model" value={formData.model || ''} onChange={handleInputChange} /></div>
                <div><label>设备编码</label><input name="device_no" value={formData.device_no || ''} onChange={handleInputChange} placeholder="如：A-12-05" /></div>
                <div><label>使用工位</label><input name="station" value={formData.station || ''} onChange={handleInputChange} /></div>
                <div><label>数量</label><input name="quantity" type="number" min="1" value={formData.quantity ?? 1} onChange={handleInputChange} /></div>
              </div>
              <div className="form-actions">
                <button type="button" onClick={() => { setShowForm(false); setEditTool(null); }}>取消</button>
      ...
      <DeleteConfirmModal
        ...
        onConfirm={async () => {
          ...
          setDeleting(true);
          try {
            await toolApi.delete(String(deleteTarget.id));
      ...
      {/* ===== 报废弹窗 ===== */}
      <ModalPortal visible={!!scrapTarget}>
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 480 }}>
            <h3 style={{ marginTop: 0 }}>工具报废</h3>
            <p style={{ margin: '4px 0 16px', color: '#555', fontSize: 13 }}>
              {scrapTarget?.name}{scrapTarget?.line ? `（${scrapTarget.line}）` : ''}&nbsp;&nbsp;
      ...
      {/* ===== 采购/报废记录弹窗 ===== */}
      <ModalPortal visible={showRecords}>
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 900, width: '92vw' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <input placeholder="搜索名称/操作人/原因" value={recordsKeyword}
